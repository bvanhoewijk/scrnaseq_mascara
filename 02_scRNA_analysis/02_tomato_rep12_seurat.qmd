...
```{r}
a <- rowMeans(GetAssayData(pbmc))

a <- data.frame(means=a)
median(a$means)
```
...
ElbowPlot2 <- function(object, ndims=20, reduction="pca"){
    ...
    plot <- ggplot(data = data.frame(dims = 1:ndims, stdev = data.use[1:ndims])) + 
        ...
        labs(x = gsub(pattern = "_$", replacement = "", x = Key(object = object[[reduction]])), 
            y = stdev) + theme_classic()
    return(plot)
}
```

```{r}
el <- (ElbowPlot2(pbmc, ndims = 50) + geom_vline(xintercept=15, col="red"))
                   
((plot1+ theme_classic()) + (plot2+ theme_classic())) /  el + plot_annotation(tag_levels = "a")
```
