...
dirs <- setNames(c("../02_scRNA_analysis/data/run_count_rep1/outs/filtered_feature_bc_matrix/", 
                   "../02_scRNA_analysis/data/run_count_rep2/outs/filtered_feature_bc_matrix/"), 
                 c("rep1", "rep2"))
...
```{r}
library(ggplot2)
rep1 <- subset(pbmc, orig.ident == "rep1")@meta.data
title1 <- paste0("Rep1: ", round(cor(rep1$nCount_RNA, rep1$percent.MT), 1))
rep2 <- subset(pbmc, orig.ident == "rep2")@meta.data
title2 <- paste0("Rep2: ", round(cor(rep2$nCount_RNA, rep2$percent.MT), 1))

p1 <- ggplot(data=rep1, aes(x=nCount_RNA, y=percent.MT)) + 
  geom_point() + 
  ggtitle(title1) +
  xlim(c(NA, max(rep1$nCount_RNA, rep2$nCount_RNA))) +
  ylim(c(NA, max(rep1$percent.MT, rep2$percent.MT)))


p2 <- ggplot(data=rep2, aes(x=nCount_RNA, y=percent.MT)) + 
  geom_point() + 
  ggtitle(title2) + 
  xlim(NA, max(rep1$nCount_RNA, rep2$nCount_RNA)) +
  ylim(NA, max(rep1$percent.MT, rep2$percent.MT))
...
pbmc_selection <- pbmc[,suberin_expression_cells]
...
pbmc_selection <- SCTransform(pbmc_selection, 
                              verbose=FALSE, 
...
cormat <- cor(counts.sub.t[,which(colnames(counts.sub.t) %in% found[1:4])])
...
```{r}
X <- resids[,-which(colnames(resids) %in% found)]
y <- resids[,which(colnames(resids) %in% found)]
...
for(i in c(1:nrep)){
  set.seed(i)
  x_index <- sample(1:ncol(X), as.integer(ncol(X)*0.8))
  y_index <- as.integer(ncol(y)*0.8)
