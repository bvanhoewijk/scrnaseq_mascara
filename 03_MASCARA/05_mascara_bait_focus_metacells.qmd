...
(lineplot + scree_plot_pls) / (p1+p2+p3) + plot_annotation(tag_levels = "a")
```

```{r, fig.width=10, fig.height=6}
baits <- found[1:4]
d <- counts.sub[c(baits, top10, bot10), ]

heatmap.2(cor(t(d)), RowSideColors = c(rep("blue", length(baits)), rep("green", 10), rep("red", 10)), 
          trace="none", 
          density.info="none",
          margins =c(8,20))
...
baits <- found[1:4]
poi <- found[5:length(found)]
top150 <- head(rownames(Candidates), 150)
top50 <- head(rownames(Candidates), 50)

annot1 <- rep("white", length(top150))
annot1[top150 %in% top50] <- "red"
annot1[top150 %in% found] <- "green"
annot1[top150 %in% baits] <- "blue"
...
heatmap.2x(cor(t(d)), 
          ...
          col=bluered(50),
          density.info="none",
          ColSideColors=annot1,
          margins =c(8,20))

legend("topright", 
       legend=c("Baits", "Related pathway", "Top 50 candidates"), 
       col=c("blue", "green", "red"), 
       lty=1, lwd=10)
```
...
baits <- found[1:4]
poi <- found
top50 <- c(head(rownames(Candidates), 50), baits)

annot1 <- rep("white", length(top50))
annot1[top50 %in% found] <- "green"
annot2 <- rep("white", length(top50))
annot2[top50 %in% baits] <- "blue"
...
```{r}
# Create color list of top20:
selection <- R[head(rownames(Candidates),100), ]

cols <- rep("gene", nrow(selection))
shape <- rep("gene", nrow(selection))

# Colors: POI, RP, Candidates
poi <- found
other <- head(rownames(Candidates), 20)

cols[rownames(selection) %in% poi] <- "POI"
shape[rownames(selection) %in% other] <- "Candidate"

to_plot =  data.frame(x=selection[, 1], y=selection[,2], group=as.factor(cols), shape=as.factor(shape))


ggplot(data=to_plot, aes(x=x, y=y, color=group, shape=shape)) + geom_point()
```
...
p1 <- ggplot(data=df1, aes(gene1, gene2)) +
  ...
  xlab(paste0("Bait: ", name1)) +
  ylab(paste0("Candidate: ", name2)) +
  ggtitle(paste0("Reconstructed MASCARA \ngene expression\nr = ", cor1))+  
  theme(legend.position = "none")
