...
meta <- data.frame(unmerged_celltype=gsub("_\\d+$", "", pbmc_unmerged$unmerged_celltype),
                   merged_celltype = pbmc_unmerged$unmerged_celltype)
...
found <- c()
for(bait in fig2e_genes){
  found <- c(found, rownames(pbmc_unmerged)[grep(bait, rownames(pbmc_unmerged))])
}
# Select suberin genes and calculate the sum of the expression
# if that sum is > 1: keep
expr <- GetAssayData(pbmc_unmerged)
suberin_expression_cells <- colSums(expr[rownames(expr) %in% found,]) > 1

pbmc_suberin <- pbmc_unmerged[,suberin_expression_cells]
